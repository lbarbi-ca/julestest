...
class Game:
    ...
    def register_listener(self, event_type: GameEventType, listener):
        self._listeners[event_type].append(listener)

    def unregister_listener(self, event_type: GameEventType, listener):
        if listener in self._listeners[event_type]:
            self._listeners[event_type].remove(listener)

    def _fire_event(self, event_type: GameEventType, **data):
    ...
    def is_card_playable(self, card: Card, hand: list[Card]) -> bool:
        top_card = self.get_top_card()
        if not top_card:
            return True

        # Wild Draw Four has a special condition
        if card.card_type == CardType.WILD_DRAW_FOUR:
            if self.current_color is None:
                return True  # Can be played if no color is active
            has_matching_color_card = any(c.color == self.current_color for c in hand)
            return not has_matching_color_card
    ...
    def play_turn(self, card_index: int | None, chosen_color: Color = None, called_uno: bool = False):
        ...
        if card_index is not None:
            ...
            if not self.is_card_playable(card_to_play, player.hand):
            ...
            # Set UNO status and fire events
            if len(player.hand) == 0:
                self._fire_event(GameEventType.GAME_WIN, winner_index=self.current_player_index)
                player.uno_status = False
            elif len(player.hand) == 1:
                player.uno_status = called_uno
                if called_uno:
                    self._fire_event(GameEventType.UNO_DECLARED, player_index=self.current_player_index)
            else:
                player.uno_status = False
    ...
    def get_playable_cards(self, player: Player) -> list[int]:
        """
        Returns a list of indices of playable cards in the player's hand.
        """
        playable_indices = []
        for i, card in enumerate(player.hand):
            if self.is_card_playable(card, player.hand):
                playable_indices.append(i)
        return playable_indices

    def player_must_draw(self, player: Player) -> bool:
        """
        Checks if the player has any playable cards. If not, they must draw.
        """
        return not self.get_playable_cards(player)
